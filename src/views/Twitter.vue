<template>
  ...
  <div class="flex container h-screen w-full">
    <!-- Side navbar -->
    <div
      class="lg:w-1/5 border-r border-t-lighter px-2 lg:px-6 py-2 flex flex-col justify-between"
    >
      <button
      ...
      <div class="lg:w-full relative">
        <button
          @click="dropdown = true"
          class="flex items-center w-full hover:bg-t-light-blue rounded-full p-2 focus:outline-none"
        >
          <img
            src="../assets/logo.png"
            alt=""
        ...
        <div
          v-if="dropdown === true"
          class="absolute bottom-0 left-0 w-64 rounded-lg
          shadow-md border-t-lightest bg-white mb-16"
        >
          <button
          ...
          <button
            @click="dropdown = false"
            class="w-full text-left hover:bg-t-lightest border-t border-t-lighter p-3 text-sm"
          >
    ...
    <div class="w-1/2 h-full overflow-y-scroll">
      <div
        class="px-5 py-3 border-b border-t-lighter flex items-center justify-between"
      >
        <h1 class="text-xl font-bold">Home</h1>
        <i class="i fas fa-start text-xl text-t-blue"></i>
      </div>
      <div class="px-5 py-3 border-b-8 border-t-lighter flex">
        <div>
          <img src="../assets/logo.png"
          alt="Profile picture"
          class="rounded-full h10 w-10 border border-t-lighter"/>
        </div>
        <form class="w-full p-4 relative">
            <textarea placeholder="Whats up ?"
            class="pb-3 mt-3 w-full focus:outline-none"></textarea>
            <div class="flex items-center">
                <i class="text-right text-t-blue mr-4 fas fa-image"></i>
                <i class="text-right text-t-blue mr-4 fas fa-film"></i>
                <i class="text-right text-t-blue mr-4 fas fa-chart-bar"></i>
                <i class="text-right text-t-blue mr-4 fas fa-smile"></i>
            </div>
            <button
            class="h-10 px-4 text-t-lighter font-semibold bg-t-blue rounded-full
            hover:bg-t-darkblue focus:outline-none absolute bottom-0 right-0">
            Tweet
            </button>
        </form>
      </div>
    ...
    <!-- right bars -->
    <div
      class="md:block hidden w-1/3 h-full
      border-l border-t-lighter py-2 px-6 overflow-y-scroll relative"
    >
      <input
        type="text"
      ...
      <div class="w-full rounded-lg bg-t-lightest p-3">
        ...
        >
          <div>
            <p class="text-sm text-left leading-tight text-t-dark">
              {{ trend.top }}
            </p>
        ...
        <button
          class="p-3 w-full hover:bg-t-lighter text-left text-t-blue border-t border-t-lighter"
        >
          Show More
        </button>
      </div>
      <div class="w-full rounded-lg bg-t-lightest my-4">
        ...
        >
          ...
          <button
            class="ml-auto text-sm text-t-blue py-2 px-4 rounded-full border-2 border-t-blue"
          >
            Follow
          </button>
        </button>
